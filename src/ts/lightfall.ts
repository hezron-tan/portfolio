...
export interface LightfallOptions {
  ...
  mixBlendMode?: string;
  dpr?: number;
  /** Element that receives pointer events for cursor light (defaults to container). */
 ...
 */
export function createLightfall(
 ...
 */
export function initBannerLightfall(): () => void {
  const banner = document.getElementById("banner");
  const mount = document.getElementById("banner-lightfall");
  if (!banner || !mount) return () => undefined;
